...
def set_window_title(title):
    try:
        ...
        print(f"Fenêtre avec le titre '{title}' non trouvée.")

set_window_title('Nom_de_la_fenetre_ppcls.py')
...
def keras_process_image(img):
    ...
    img = np.repeat(img.reshape(-1, image_x, image_y, 1), 3, axis=-1)
    return img

def get_emojis():
    emojis_folder = 'RPS_emo/'
    emojis = []
    for emoji in range(3):  # Modification ici pour utiliser uniquement les classes 0, 1, 2
        print(emoji)
...
keras_predict(model, np.zeros((50, 50, 3), dtype=np.uint8))
main()
